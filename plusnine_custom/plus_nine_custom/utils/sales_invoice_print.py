...
def get_inv_item_and_tax_details(voucher_type, voucher_no, hsn_wise = False):
    ...
    for row in items:
        if row.item_code in itemised_tax:
            ...
            tax_percent = row.gst_hsn_code #f"{(row.get('cgst') or 0) + (row.get('sgst') or 0) + (row.get('igst') or 0)} {row.gst_hsn_code if hsn_wise else ''}" 
    ...
    if voucher_type == "Sales Invoice":
          total.append({
            'tax_percentage': "18.0" if total_igst else "9.0",
            'gst_hsn_code': "86090000",
    ...
    total.append({
            ...
            'taxable_amount': doc.net_total+frieght_amount,
            'cgst': total_cgst+(frieght_tax/2),
            'sgst': total_sgst+(frieght_tax/2),
            'igst': total_igst+(frieght_tax),
            'total_tax_amount': (total_cgst or 0) + (total_sgst or 0) + (total_igst or 0) +frieght_tax
        })
    return {
        "items": items,
        "instate": instate,
